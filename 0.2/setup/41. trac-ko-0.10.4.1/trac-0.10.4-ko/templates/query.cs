...
<div id="ctxtnav" class="nav"><?cs
 if:query.report_href ?><ul>
  <li class="first"><a href="<?cs
    var:query.report_href ?>">이용 가능한 리포트들</a></li>
  <li class="last">맞춤형 쿼리</li></ul><?cs
 /if ?>
</div>

<?cs def:num_matches(v) ?><span class="numrows">(<?cs 
 alt:v ?>No<?cs /alt ?> 개 일치함)</span><?cs
/def ?>
...
<form id="query" method="post" action="<?cs var:trac.href.query ?>">
  ...
  <table summary="Query filters">
   ...
   <tbody><tr class="actions">
    <td class="actions" colspan="4" style="text-align: right">
     <label for="add_filter">필터 추가하기</label>&nbsp;
 ...
 <p class="option">
  ...
  <label for="verbose">각 결과의 전체 내용보기</label>
 </p>
 <div class="buttons">
 ...
 each:result = query.results ?><?cs
  ...
  each:header = query.headers ?><?cs
   if:name(header) == 0 ?><td class="id"><a href="<?cs
    var:result.href ?>" title="티켓 보기"><?cs var:result.id ?></a></td><?cs
   else ?><td class="<?cs var:header.name ?>"><?cs
     if:header.name == 'summary' ?><a href="<?cs
      var:result.href ?>" title="티켓 보기"><?cs
  ...
  <?cs if:query.verbose ?>
    ...
    <p class="meta">작성자 <strong><?cs var:result.reporter ?></strong>,
...
<div id="help">
 <strong>참고 :</strong> 쿼리를 사용하면서 도움이 필요하다면, <a href="<?cs var:trac.href.wiki ?>/TracQuery">TracQuery</a> 페이지를 참고하십시오.
</div>
