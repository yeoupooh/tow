...
""")
    ...
    def get_navigation_items(self, req):
        ...
        yield ('mainnav', 'newticket', 
               html.A(u'새로운 티켓', href=req.href.newticket(), accesskey=7))
...
class TicketModule(TicketModuleBase):
    ...
    def get_timeline_filters(self, req):
        if req.perm.has_permission('TICKET_VIEW'):
            yield ('ticket', u'티켓 상태의 변경사항')
            if self.timeline_details:
                yield ('ticket_details', u'티켓 내용의 변경사항', False)

    def get_timeline_events(self, req, start, stop, filters):
        format = req.args.get('format')

        status_map = {'new': ('newticket', u'생성됨'),
                      'reopened': ('newticket', u'다시열림'),
                      'closed': ('closedticket', u'닫힘'),
                      'edit': ('editedticket', u'갱신됨')}
        ...
        def produce((id, t, author, type, summary), status, fields,
            ...
            if format == 'rss':
                ...
                title = Markup(u'티켓 <em title="%s">#%s</em> (%s) : %s에 의해 %s',
