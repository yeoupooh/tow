...
class ReportModule(Component):
    ...
    def get_navigation_items(self, req):
        ...
        yield ('mainnav', 'tickets',
               html.A(u'티켓들 보기', href=req.href.report()))
    ...
    def process_request(self, req):
        ...
        if id != -1 or action == 'new':
            add_link(req, 'up', req.href.report(), u'이용 가능한 리포트들')
    ...
    def _render_confirm_delete(self, req, db, id):
        ...
        row = cursor.fetchone()
        if not row:
            raise TracError(u'%s번 리포트는 존재하지 않습니다.' % id,
                            u'유효하지 않은 리포트 번호')
    ...
    def get_info(self, db, id, args):
        if id == -1:
            ...
            title = u'이용 가능한 리포트들'
            sql = u'SELECT id AS report, title FROM report ORDER BY report'
            description = u'이용 가능한 리포트들의 리스트입니다.'
        else:
            cursor = db.cursor()
            ...
            row = cursor.fetchone()
            if not row:
                raise TracError(u'%d번 리포트는 존재하지 않습니다.' % id,
                                u'유효하지 않은 리포트 번호')
            title = row[0] or ''
            sql = row[1]
