...
class WikiModule(Component):
    ...
    def get_navigation_items(self, req):
        if not req.perm.has_permission('WIKI_VIEW'):
            return
        yield ('mainnav', 'wiki',
               html.A(u'위키', href=req.href.wiki(), accesskey=1))
        yield ('metanav', 'help',
               html.A(u'도움말/가이드', href=req.href.wiki('TracGuide'),
                      accesskey=6))
    ...
    def get_timeline_events(self, req, start, stop, filters):
        if 'wiki' in filters:
            ...
            for t,name,comment,author,version in cursor:
                title = Markup(u'<em>%s</em> : %s에 의해 수정됨',
    ...
    def _render_history(self, req, db, page):
        ...
        if not page.exists:
            raise TracError, u"%s 페이지가 존재하지 않습니다." % page.name
    ...
    def _render_view(self, req, db, page):
        ...
        if page.exists:
            ...
            req.hdf['wiki.page_html'] = html.P(u'여기에서 "%s" 페이지를 작성하십시오.' % page_name)
